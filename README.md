...
```java
// smack-core/src/main/java/org/jivesoftware/smack/proxy/HTTProxySocketConnection.java
...
class HTTPProxySocketConnection implements ProxySocketConnection {
  ...
  public void connect(Socket socket, String host, int port, int timeout) {
    ...
    socket.getOutputStream().write((hostport + " HTTP/1.1/r\nHost: "
        + host + ":" + port + proxyLine + "\r\n\r\n").getBytes("UTF-8"));
    ...
    if (!m.matches()) {
      throw new ProxyExceptoin(ProxyInfo.ProxyType.HTTP , "Unexpected " +
  ...
  private static final Pattern RESPONSE_PATTERN
    = Pattern.compile("HTTP/\\S+\\s(\\d+)\\d(.*)\\s*");
